"""Skeptic agent that argues AGAINST product compatibility in the debate system.

This agent takes the position that products may not be compatible and looks for
evidence to support that conclusion. It provides the opposing side of the debate
that will be resolved by the judge agent.

Design Philosophy:
- Focus on finding incompatibility evidence
- Challenge assumptions about compatibility
...
class SkepticAgent(DebateAgent):
    """Agent that argues AGAINST compatibility between products.

    This agent actively looks for evidence that products may not be compatible
    or identifies potential issues. It provides the skeptical perspective in the
    ...
    def __init__(self, llm_service: LLMService):
        ...
        super().__init__(
            ...
            prompt_name="compatibility_agent_against",
        )
    ...
    def evaluate(self, context: AgentContext) -> AgentResult:
        """Evaluate compatibility by arguing AGAINST compatibility.

        Args:
            context: Evaluation context with product information

        Returns:
            AgentResult: Structured evaluation arguing against compatibility
        ...
        try:
            ...
            raise ValueError(f"Skeptic agent returned invalid JSON: {e}") from e

        except Exception as e:
            logger.error(
                "skeptic_evaluation_failed",
                extra={
                    "agent_name": self.name,
            ...
            raise RuntimeError(f"Skeptic agent evaluation failed: {e}") from e
    ...
    def _parse_llm_response(self, raw_output: str, request_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug(
                "skeptic_json_parsed_successfully",
                extra={
                    "agent_name": self.name,
